...
const ActivitiesPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="bg-gradient-to-r from-primary/90 to-primary sticky top-0 z-20 shadow-md">
        <div className="container mx-auto px-4 py-4 flex flex-col sm:flex-row sm:items-center justify-between">
          ...
          <div className="hidden sm:flex items-center gap-6">
            ...
            <Link to="/activities" className="text-white hover:text-white/80 font-medium transition-colors px-2 py-1 rounded hover:bg-white/10 bg-white/20">Activities</Link>
      ...
      {/* Hero Banner */}
      <div className="relative h-64 md:h-80 bg-cover bg-center" style={{ backgroundImage: 'url(https://images.pexels.com/photos/1271619/pexels-photo-1271619.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1)' }}>
        <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="text-center text-white px-4">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Explore Activities</h1>
            <p className="text-xl max-w-2xl mx-auto">Discover amazing experiences and adventures</p>
          </div>
        </div>
      </div>
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
            ...
            <Button 
              ...
              className="rounded-full"
              onClick={() => handleFilterChange('hotel', 'all', '')}
            >
              Hotels & Resorts
            </Button>
            <Button 
              ...
              className="rounded-full"
              onClick={() => handleFilterChange('restaurant', 'all', '')}
            >
              Restaurants
            </Button>
            <Button 
              variant={selectedCategory === 'saloon' ? 'default' : 'outline'} 
              className="rounded-full"
              onClick={() => handleFilterChange('saloon', 'all', '')}
            >
              Saloons & Spas
            </Button>
            <Button 
              variant={selectedCategory === 'lodge' ? 'default' : 'outline'} 
              className="rounded-full"
              onClick={() => handleFilterChange('lodge', 'all', '')}
            >
              Lodges
            </Button>
            <Button 
              ...
              className="rounded-full"
              onClick={() => handleFilterChange('adventure', 'all', '')}
            >
              Adventure
            </Button>
          </div>
